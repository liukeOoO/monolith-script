...
# install java --> temurin-8-kdk, https://adoptium.net/installation/linux/
RUN set -eux; \
...
RUN wget https://download.open-mpi.org/release/open-mpi/v4.0/openmpi-4.0.0.tar.gz && \
    ...
    cd .. && rm -rf openmpi-4.0.0.tar.gz openmpi-4.0.0
...
RUN HOROVOD_WITH_TENSORFLOW=1 HOROVOD_WITH_MPI=1 HOROVOD_WITHOUT_GLOO=1 HOROVOD_CPU_OPERATIONS=MPI HOROVOD_GPU_OPERATIONS=NCCL python3.8 -m pip install --no-cache-dir horovod
...
RUN wget -q https://archive.apache.org/dist/hadoop/common/hadoop-3.3.2/hadoop-3.3.2.tar.gz && \
    tar -xzf hadoop-3.3.2.tar.gz && mv hadoop-3.3.2 /opt/tiger/hadoop && rm hadoop-3.3.2.tar.gz
...
RUN printf "\nexport PATH=${JAVA_HOME}/bin:${MPI_HOME}/bin:${CUDA_HOME}/bin:$HADOOP_HOME/sbin:$HADOOP_HOME/bin:$PATH\n" >> /root/.bashrc && \
    printf "\nexport PATH=${JAVA_HOME}/bin:${MPI_HOME}/bin:${CUDA_HOME}/bin:$HADOOP_HOME/sbin:$HADOOP_HOME/bin:$PATH\n" >> /home/tiger/.bashrc
